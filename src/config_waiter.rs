...
pub fn wait_for_partition_boot(config_store: Arc<RwConfigStore>, partition: u16) -> Receiver<()> {
    ...
    tokio::spawn(async move {
        ...
        tracing::info!(
            ...
            total_configs,
            partition,
            "partition_boot_complete"
        );

        metrics::gauge!("config_consumer.partition_boot_time_ms").set(boot_time_ms as f64);
        metrics::gauge!("config_consumer.partition_total_configs").set(total_configs as f64);
