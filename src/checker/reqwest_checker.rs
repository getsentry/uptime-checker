...
impl Checker for ReqwestChecker {
    ...
    async fn check_url(
        ...
        let trace_id = make_trace_id(config, tick, retry_num);
...
mod tests {
    ...
    async fn test_default_get() {
        ...
        let tick = make_tick();
        let result = checker.check_url(&config, &tick, "us-west", 0).await;
    ...
    async fn test_restricted_resolution() {
        ...
        let tick = make_tick();
        let result = checker
            .check_url(&localhost_config, &tick, "us-west", 0)
            .await;
    ...
    async fn test_validate_url() {
        ...
        let tick = make_tick();
        let result = checker
            .check_url(&restricted_ip_config, &tick, "us-west", 0)
            .await;
        ...
        let tick = make_tick();
        let result = checker
            .check_url(&restricted_ipv6_config, &tick, "us-west", 0)
            .await;
        assert_eq!(
            result.status_reason.map(|r| r.description),
            Some("destination is restricted".to_string())
    ...
    async fn test_same_check_same_id() {
        ...
        let tick = make_tick();
        let result = checker.check_url(&config, &tick, "us-west", 0).await;
        let result_2 = checker.check_url(&config, &tick, "us-west", 0).await;
